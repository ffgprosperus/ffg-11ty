...
module.exports = (pageNav, page, maxLevel = 999) => {
  ...
  function navRecurse(entry, level = 1) {
    ...
    return (
      '<li' +
      (classList.length ? ` class="nav-item ${ classList.join(' ') }"` : '') +
      '>' +
      (active ? `<${ elementActive }>` : `<a class="nav-link" href="${ entry.url }">`) +
      entry.title +
  ...
  return `<${ listType } class="navbar-nav nav-fill w-100">${ nav }</${ listType }>`;
