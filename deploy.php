 ...
 * @var string
 */
if (!defined('SECRET_ACCESS_TOKEN')) define('SECRET_ACCESS_TOKEN', 'ProsperUS');
...
//define('NPM_BINARY', '/home/dh_3abdz7/.nvm/versions/node/v14.16.1/bin/npm');
...
// The loop of commands below will change the working directory, so we save the current directory before executing
// This is needed to switch back to run deploy.sh
$original_dir = getcwd();
...
//Deploy.sh dumps any errors to output.txt and error.txt in the directory where the code is pulled 
//because github will give up on displaying the response to the webhook long before it's done
printf('Running the deploy script');
//Swapping back to the original directory (saved above this loop) so we can find deploy.sh
chdir($original_dir);
// Setting FFG_TMP_DIR an environment variable so deploy.sh can find the pulled code from Github
exec('FFG_TMP_DIR="'.TMP_DIR.'" ./deploy.sh 2>&1', $output);
// Dump any logs or errors if it quits prematurely (if the hook fails because of timeout, it's actually working)
var_dump($output);
